...
let show_team_list team_list =
  ...
  (** [show_player_stat] returns the player statistics *)
  let show_player_stat player_stat = 
    start_state (size_y ());
    let _ = set_window_title "Player Statistics" in 
    let button_list = make_button_list player_stat in
    let () = set_color black in 
      let _ = print_name "Statistics of Player" (current_x ()) (current_y ()) max_horz
